...
class String {
	...
	public static function uuid($salt=null) {
		...
		if (empty($node)) {
            if (is_null($salt) && !function_exists('get_instance')) {
                throw new Exception('No Salt passed');
            }

            if (function_exists('get_instance')) { // CI application
                $CI =& get_instance();
                $salt = $CI->config->item['salty_salt'];
            }
            $node = crc32($salt);
		}
